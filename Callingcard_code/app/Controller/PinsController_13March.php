...
// Create new PHPExcel object
	...
	public function admin_pins_detail_excel($card_id = 0,$cat_id = 0, $sub_cat_id = 0, $pc_status = ''){
		...
		if(isset($sub_cat_id) && !empty($sub_cat_id) && in_array($sub_cat_id,array_keys($resSubCat))) {
			$cardCondition['c_cat_id'] = $sub_cat_id;
		}

		if(isset($card_id) && !empty($card_id)){
			$cardCondition['c_id'] = $card_id;
			$cardCondition['c_status'] = 1;
		} 
		...
		foreach ($cardRes as $key => $value) 
		{
			...
			if(!empty($value['PinsCard']))
			{

				foreach ($value['PinsCard'] as $k => $v) 
				{
					if($v['pc_status'] == 2)
					$sold_pin++;
					else
					...
					if(isset($pc_status) && empty($pc_status))
					{
						...
						$pinRes = $this->Pin->find('all',array(
								'conditions' => array('Pin.p_id'=>$v['pc_p_id']),
								'fields' => array('*'),
								//'recursive'	 => -1,
								//'order'=>'c_title'
							));
					...
					if(!empty($pinRes[0]['Pin']))
					$cardRes[$key]['Pin_detail'][] = $pinRes[0]['Pin'];
				}
			}
		}
		...
		else
		{
			foreach($cardRes as $keynew=>$val)
			{
				if($pc_status=='')
				{
                    ...
                    //prd($val['Card']['c_id']);
					$card_detail = "Card Name: ".$val['Card']['c_title']."   Pin Sold: ".$val['Card']['pin_card_sold_count']."   Remaining Pins: ".$val['Card']['pin_card_remain_count'];
					//$card_detail = "Card Name: ".$val['Card']['c_title']."   Pin Sold: ".$sold_pin."   Remaining Pins: ".$other_pin;
					...
					//$objPHPExcel->getActiveSheet()->setCellValue('D'.$heading,"FileName");
					//$objPHPExcel->getActiveSheet()->setCellValue('E'.$heading,"Created Date");
					...
					if(isset($val['Pin_detail']) && !empty($val['Pin_detail'])){
						...
						foreach ($val['Pin_detail'] as $key => $value) 
						{
							...
							/*if($value['p_status']==0){
								$objPHPExcel->getActiveSheet()->setCellValue('F'.$FirstItemNumber,'All');
							}*/
							if($value['p_status']==1){
								$objPHPExcel->getActiveSheet()->setCellValue('F'.$FirstItemNumber,'Unused');
							}
							if($value['p_status']==2){
								$objPHPExcel->getActiveSheet()->setCellValue('F'.$FirstItemNumber,'Sold');
							}
							if($value['p_status']==3){
								$objPHPExcel->getActiveSheet()->setCellValue('F'.$FirstItemNumber,'Parked');
							}
							if($value['p_status']==4){
								$objPHPExcel->getActiveSheet()->setCellValue('F'.$FirstItemNumber,'Rejected');
							}
							if($value['p_status']==5){
								$objPHPExcel->getActiveSheet()->setCellValue('F'.$FirstItemNumber,'Returned');
							}
					...
					else
					{
						...
						//$objPHPExcel->getActiveSheet()->getStyle('C'.$FirstItemNumber.':G'.$FirstItemNumber)->getFont()->setBold(true);
						$objPHPExcel->getActiveSheet()->mergeCells('D'.$FirstItemNumber.':G'.$FirstItemNumber);
						$objPHPExcel->getActiveSheet()->getStyle('A'.$FirstItemNumber.':G'.$FirstItemNumber)->getFont()->setBold(false)->setSize(11);
						$objPHPExcel->getActiveSheet()->getStyle('A'.$FirstItemNumber.':G'.$FirstItemNumber)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
						$objPHPExcel->getActiveSheet()->getStyle('A'.$FirstItemNumber.':G'.$FirstItemNumber)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);

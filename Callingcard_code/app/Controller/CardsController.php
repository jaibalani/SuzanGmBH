...
class CardsController extends AppController
{
	...
	public function admin_index()
	{
		...
		if($resSubCat)
		{
			$cardRes = $this->Card->find('list',array(
				'conditions' => array('Card.c_cat_id'=>array_keys($resSubCat)),
				'fields' => array('c_id','c_title'),
				'recursive'	 => -1,
				'order'=>'c_title'
			));
	...
	public function admin_json($id=null)
	{
		...
		if($this->request->is('ajax'))
		{
			...
			'all_pins_except_sold' => '
				                 Card.pin_card_remain_count+
								 Card.pin_card_count_parked+
								 Card.pin_card_count_rejected+
								 Card.pin_card_count_returned'				
			...
			if(isset($this->request->query['filters'])){
				...
				foreach($filters['rules'] as $each_filter){
					...
					if($each_filter['field'] == 'Parent.cat_title'){
						$conditions['Parent.cat_title LIKE'] = '%'.Sanitize::escape($each_filter['data']).'%';
					   
					}


					if($each_filter['field'] == 'all_pins_except_sold')
					{
						$conditions['Card.all_pins_except_sold LIKE'] = '%'.Sanitize::escape($each_filter['data']).'%';
					}
					
					if($each_filter['field'] == 'c_status'){
						if($each_filter['data'] != '-1')
			...
			foreach($resultSet as $key=>$val){
				...
				//prd($val);
				$pins_count_delete = count($val['PinsCard']);
                $sales_count_delete = $this->Sale->find('count',array('conditions'
                					=>array('s_c_id'=>$val['Card']['c_id'])));
				...
				{
					$pins_manage = '<a title="You can not view the pins details of disabled cards" onclick="view_alert();"><img src="'.$this->webroot.'img/detail.png" alt="Manage Cards"  border="0" /> ('.$val['Card']['pin_card_count'].') </a>';
				}
				...
				//echo $sales_count_delete." ".$pins_count_delete;
				if($sales_count_delete === 0 && $pins_count_delete === 0)
				{
					$del = '<a title="Delete Card" onclick="delete_card('.$val['Card']['c_id'].')" style="cursor:pointer; color:#222222;">Delete</a>';
				}	
				...
				{
					$action = "&nbsp;".$edit." ".$import_pins." ".$export_pins." ".$merge_pins." ".$unmerge_pins;
				}
    ...
    public function admin_manage_price($char_code = 'All',$mediator_id = 0,$selected_card_category = 0,$selected_sub_category = 0,$card_rate = 0,$card_id = 0,$overrideen = 0){
		...
		if(isset($card_rate) && !empty($card_rate)) {
		    ...
		    {
				$card_price_conditions['Card.c_buying_price >'] = 1;
				$card_price_conditions['Card.c_buying_price <='] = 2.5;
			}
			else if($rate == 3) 	
		    {
				$card_price_conditions['Card.c_buying_price >'] = 2.5;
    ...
    public function admin_manage_price_mediator($char_code = '0',$retailer_id = 0,$selected_card_category = 0,$selected_sub_category = 0,$card_rate = 0,$card_id=0,$overridden = 0){
		...
		$price_range = "> 0";
		//$card_price_conditions['CardsPrice.cp_buying_price >'] = 0;
		if(isset($card_rate) && !empty($card_rate)) {
			...
			if($rate == 1)
			$price_range = "<= 1";
			//$card_price_conditions['CardsPrice.cp_buying_price <'] = 1;
		
			else if($rate == 2) 	
		    {
		    	$price_range = "between 1.01 and 2";
				//$card_price_conditions['CardsPrice.cp_buying_price >='] = 1;
				//$card_price_conditions['CardsPrice.cp_buying_price <='] = 2;
			}
			else if($rate == 3) 	
		    {
				$price_range = "between 2.51 and 5";
				//$card_price_conditions['CardsPrice.cp_buying_price >='] = 2;
				//$card_price_conditions['CardsPrice.cp_buying_price <='] = 5;
			}
			else if($rate == 4) 	
		    {
		    	$price_range = "between 5.01 and 10";
				//$card_price_conditions['CardsPrice.cp_buying_price >='] = 5;
				//$card_price_conditions['CardsPrice.cp_buying_price <='] = 10;
			}
			else if($rate == 5) 	
		    {
				$price_range = "> 10";
        ...
        $card_conditions_new = 'Card.c_status = 1 ';
        if(isset($card_price_conditions['Card.c_cat_id']))
        {
        	if(is_array($card_price_conditions['Card.c_cat_id']))
        	{
              $cat = implode(',', $card_price_conditions['Card.c_cat_id']);
        	  $card_conditions_new .= ' and Card.c_cat_id in ('.$cat.')';
        	}
        	else
        	{
              $cat = $card_price_conditions['Card.c_cat_id'];
        	  $card_conditions_new .= ' and Card.c_cat_id = '.$cat;
        	}
        }  

        if(isset($card_price_conditions['Card.c_id']))
        {
        	$card_conditions_new .= ' and Card.c_id = '.$card_price_conditions['Card.c_id'].'';
        } 
        
        
        if(isset($card_price_conditions['Card.c_title LIKE']))
        {
        	$card_conditions_new .= "and Card.c_title LIKE '".$card_price_conditions['Card.c_title LIKE']."'";
        } 
        
        $card_conditions_new_buying = $card_conditions_new.' and CardsPrice.cp_buying_price '.$price_range;
        $card_conditions_new_selling = $card_conditions_new.' and CardsPrice.cp_selling_price '.$price_range; 


        $card_price_case = 'CASE
			WHEN  EXISTS (
				           SELECT * FROM ecom_cards_prices as CardsPrice
			               WHERE 
			              (
	...
	public function manage_price_retailer($selected_card_category = 0,$char_code = 0,$selected_sub_category = 0){
		...
		if($selected_card_category != '0')
		{
		  $get_selected_child_data = $this->Category->find('first',array('conditions'=>array('Category.cat_id'=>$selected_card_category,'Category.cat_status'=>1),'order'=>'Category.cat_title asc'));	
          //pr($get_selected_child_data);
		  $all_sub_category = array();
		  if(!empty($get_selected_child_data['Child']))
		  {
              foreach($get_selected_child_data['Child'] as $data)
			  {
	...
	public function admin_card_inventory($cat_id = 0, $sub_cat_id= 0,$card_id = 0)
	{
        ...
        $counter = 0;
        $inventory_report_category = array();
		foreach($res as $key => $value)
		{
            $res[$key] = ucwords($value);
		    $subCatConditions = array();
            ...
            if(empty($resSubCat))
            {
                 $inventory_report_category[$counter]['main_cat'] = '';
            	 $inventory_report_category[$counter]['main_cat_title'] = ucwords(strtolower($value));
            	 $inventory_report_category[$counter]['sub_cat'] = '';
            	 $inventory_report_category[$counter]['sub_cat_title'] = __('There is no sub category in this category.');
            	 $inventory_report_category[$counter]['chnage_cat'] = 1;
            	 $counter++;
            }
            else
            {
                ...
                {
                	 $resSubCat[$sub_id] = ucwords(strtolower($sub_title));
                	 if($new_sub_cat == 0)
                	 {
                	 	$new_sub_cat = 1;
                	 ...
                	 {
                	 	 $inventory_report_category[$counter]['chnage_cat'] = 0;
                	 }
                	 $inventory_report_category[$counter]['main_cat'] = $key;
                	 $inventory_report_category[$counter]['main_cat_title'] = ucwords(strtolower($value));
                	 $inventory_report_category[$counter]['sub_cat'] = $sub_id;
                	 $inventory_report_category[$counter]['sub_cat_title'] = ucwords(strtolower($sub_title));
                     $counter++;
                }
            }
		}
		...
		if(!empty($cat_id) && $cat_id > 0)
		{
			...
			{
				foreach ($inventory_report_category as $key => $value) {
                	if($cat_id != $value['main_cat'])
               		unset($inventory_report_category[$key]);
                }
        ...
        foreach($resSubCat as $s =>$t)
        {
        	$resSubCat[$s] = ucwords(strtolower($t));
        }
        ...
        $get_cards = $this->Card->find('list',array('conditions'=>$card_conditions,
        											'order'=>$order,
        											'fields'=>$fields));
		//pr($get_cards);
		$all_cards =array();
		...
		$card_fields = array('Card.c_title',
							  'Card.c_pin_per_card',
							  'Card.sale_count',
							  'Card.pin_card_count',
							  'Card.pin_card_sold_count',
							  'Card.pin_card_remain_count',
							  'Card.c_id',
							  'Category.cat_title',
        ...
        foreach ($inventory_report_category as $key => $value) 
	    {
			$inventory_data_category_wise[$counter]['main_cat'] = $value['main_cat'];
			$inventory_data_category_wise[$counter]['main_cat_title'] = $value['main_cat_title'];
			$inventory_data_category_wise[$counter]['sub_cat'] = $value['sub_cat'];
			$inventory_data_category_wise[$counter]['sub_cat_title'] = $value['sub_cat_title'];
            $inventory_data_category_wise[$counter]['chnage_cat'] = $value['chnage_cat'];
            ...
            if(!empty($value['sub_cat']))
            {
               ...
               $get_card_details = $this->Card->find('all',array(
               						'conditions'=>$card_conditions,
               						'order'=>$order,
               						'recursive'=>'-1',
               						'fields'=>$card_fields));
			...
			{
			   $final_card_array[$counter_card]['id']    = $details['Card']['c_id'];
			   $final_card_array[$counter_card]['title'] = ucwords(strtolower($details['Card']['c_title']));
		       $final_card_array[$counter_card]['card_sold'] = intval($details['Card']['pin_card_sold_count'] / $details['Card']['c_pin_per_card']);
			   $final_card_array[$counter_card]['card_remaining'] = intval($details['Card']['pin_card_remain_count'] / $details['Card']['c_pin_per_card']);
			   $final_card_remain = $final_card_array[$counter_card]['card_remaining'] + $final_card_remain;
		       $final_card_sold = $final_card_array[$counter_card]['card_sold'] + $final_card_sold;
			   $counter_card++; 
			}
            $inventory_data_category_wise[$counter]['card_data'] = $final_card_array;
            $inventory_data_category_wise[$counter]['card_sold'] = $final_card_sold;
            $inventory_data_category_wise[$counter]['card_remaining'] = $final_card_remain;
		...
		$this->set('inventory_data_category_wise',$inventory_data_category_wise);
	}
	...
	public function  admin_excel_card_inventory($cat_id=0,$sub_cat_id = 0,$card_id = 0){
		...
		$all_cards_sold = 0;
		$all_cards_remain = 0;
		foreach($inventory_data_category_wise as $keynew => $val)
		{
			if($val['chnage_cat'] == 1)
			{
				if($counter_start == 0)
				...
				if($keynew != 0)
				{
					...
					$total_sales_details = __('Total Cards Sold')." : ". $total_cards_sold."  ".__('Total Cards Remaining')." : ". $total_cards_remain ;  
					$total_cards_sold =0;
	                $total_cards_remain =0;
					// Merging Column
			 ...
			 $total_cards_sold =$total_cards_sold+$val['card_sold'];
             $total_cards_remain =$total_cards_remain+$val['card_remaining'];

             $all_cards_sold =$all_cards_sold+$val['card_sold'];
             $all_cards_remain =$all_cards_remain+$val['card_remaining'];
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('A'.$FirstItemNumber,'S.No');
			$objPHPExcel->getActiveSheet()->setCellValue('B'.$FirstItemNumber,"Category");
			$objPHPExcel->getActiveSheet()->setCellValue('C'.$FirstItemNumber,"Sub Category");
			$objPHPExcel->getActiveSheet()->setCellValue('D'.$FirstItemNumber,"Card");
			$objPHPExcel->getActiveSheet()->setCellValue('E'.$FirstItemNumber,'Total Sold');
			$objPHPExcel->getActiveSheet()->setCellValue('F'.$FirstItemNumber,"Remaining");

            $FirstItemNumber++; 
			$serial_counter = 0;
            foreach ($val['card_data'] as $keyCard => $card_val) 
            {
            	$serial_counter++;
                 
	            $objPHPExcel->getActiveSheet()->setCellValue('A'.$FirstItemNumber,$serial_counter);
				$objPHPExcel->getActiveSheet()->setCellValue('B'.$FirstItemNumber,$val['main_cat_title']);
				$objPHPExcel->getActiveSheet()->setCellValue('C'.$FirstItemNumber,$val['sub_cat_title']);
				$objPHPExcel->getActiveSheet()->setCellValue('D'.$FirstItemNumber,$card_val['title']);
				$objPHPExcel->getActiveSheet()->setCellValue('E'.$FirstItemNumber,$card_val['card_sold']);
				$objPHPExcel->getActiveSheet()->setCellValue('F'.$FirstItemNumber,$card_val['card_remaining']);
			...
			$objPHPExcel->getActiveSheet()->setCellValue('E'.$FirstItemNumber,$val['card_sold']);
			$objPHPExcel->getActiveSheet()->setCellValue('F'.$FirstItemNumber,$val['card_remaining']);
		}
		
		if(!empty($inventory_report_category))
		{
			$FirstItemNumber++;
			$objPHPExcel->getActiveSheet()->getStyle('A'.$FirstItemNumber.':F'.$FirstItemNumber)->applyFromArray($styleThinBlackBorderOutline);
		    $objPHPExcel->getActiveSheet()->getStyle('A'.$FirstItemNumber.':F'.$FirstItemNumber)->getFont()->setBold(True)->setSize(11);
			...
			$total_sales_details = __('Final Details').' -- '.__('Total Cards Sold')." : ". $all_cards_sold."  ".__('Total Cards Remaining')." : ". $all_cards_remain ;  
			// Merging Column
	    	$objPHPExcel->getActiveSheet()->mergeCells('A'.$FirstItemNumber.':F'.$FirstItemNumber);
	    	$objPHPExcel->getActiveSheet()->setCellValue('A'.$FirstItemNumber,$total_sales_details);
	...
	public function admin_delete_card($card_id){

        $this->admin_redirect_to_dashboard_distributor(); 
        $card_details =$this->Card->findByCId($card_id);
		if($card_details)
		{
			$card_name = ucwords(strtolower($card_details['Card']['c_title']));
			
			$delete_card = $this->Card->delete($card_id);
	        if($delete_card)
	        {
	          $this->Session->setFlash(__('You have successfully deleted the card <b>'.$card_name),'default', array('class'=>'success'));
	        } 
	        else
	        {
	          $this->Session->setFlash(__('You could not delete the card <b>'.$card_name.'</b> now. Please try again later.'), 'default', array('class'=>'error'));
	        }
		} 
		else
		{
          $this->Session->setFlash(__('Invalid card selection.'),'default', array('class'=>'error'));
		}

...
-->
	<div class="sub_title">
		...
		<i class="icon-angle-right home_icon"></i> <span>Sales Report</span>
	</div>
...
$sales_counter = 1;
           ...
           <tr class="sum">
						...
						<td data-total="<?php echo $data['Sale']['s_total_sales']; ?>"><?php echo "&euro;".$data['Sale']['s_total_sales']; ?></td>
...
$(document).ready(function(){
   ...
   $('#sales_report').addClass('sb_active_subopt_active');
              ...
              filteredRows: function(filterStates) {
                ...
                $('.sum').each(function() {
                    if(!$(this).attr("filtermatch")){
                        ...
                        var t = parseFloat($(this).find('td').eq(5).data('total'));
                        sumQuantity += p;
                        sumTotal += t;
                    }
                });
...
$('#filter_card_button').click(function(){
 ...
 {
 	var url = "<?php echo $this->Html->url(array('controller'=>'Reports','action'=>'sales_report_distributor'));?>/"+mediator_id+"/"+retailer_id+"/"+card_id+"/"+url_start_date+"/"+url_end_date+ "/" + cat_id + "/" + sub_cat_id;
 }
 else
 {
    url_start_date = null;
...
/*	if(start_timestamp > end_timestamp)
 ...
 if(length_start != 0 && length_end !=0)
 {
 	var url = "<?php echo $this->Html->url(array('controller'=>'Reports','action'=>'sales_report_excel_distributor'));?>/"+cat_id+ "/" + sub_cat_id + "/" +mediator_id+ "/" +retailer_id+"/"+card_id+"/"+url_start_date+"/"+url_end_date;
 }
 else
 {
 	var url = "<?php echo $this->Html->url(array('controller'=>'Reports','action'=>'sales_report_excel_distributor'));?>/"+cat_id+ "/" + sub_cat_id + "/" +mediator_id+"/"+retailer_id+"/"+card_id;
 }
 window.location.href = url;
...
function single_excel_report(sales_id)
{
	...
	var url_end_date = 0;
	var url = "<?php echo $this->Html->url(array('controller'=>'Reports','action'=>'sales_report_excel_distributor'));?>/"+cat_id+ "/" + sub_cat_id + "/" +mediator_id+"/"+retailer_id+"/"+card_id+"/"+url_start_date+"/"+url_end_date+"/"+sales_id;
    window.location.href = url;
}
...
$('.clear_filer_class').click(function(){
	var new_url = "<?php echo $this->Html->url(array('controller'=>'Reports','action'=>'sales_report_distributor','admin'=>'true'));?>";
